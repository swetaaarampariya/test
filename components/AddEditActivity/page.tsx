...
import { AddEditActivityProps } from "@/common/constants/interface";

const AddEditActivity: FC<AddEditActivityProps> = ({ toggleModal, title, collapsed, data, setTableFilter, tableData }) => {
  ...
  const handleSubmit = async (values: { activityName: string }, { resetForm, setSubmitting }: any) => {
    try {
      const payload = { id: data?.id, activityName: values?.activityName };
      const response = title
        ? await AxiosPatch(`${API_URLS.ACTIVITY.UPDATE}`, payload)
        : await AxiosPost(`${API_URLS.ACTIVITY.ADD}`, payload);
  ...
  const existingActivity = tableData?.data?.map((item:any) => item.activityName) || [];
  const ActivityValidation = Yup.object().shape({
    activityName: Yup.string()
      ...
      .test(
        ...
        (value) => {
          if (data?.id && value === data?.activityName) {
            return true;
          }
          if (value && !existingActivity.includes(value)) {
            return true;
          }
        }
      )
  ...
  return (
    <div
      className={`flex items-center justify-center z-[1000] p-4 overflow-x-hidden overflow-y-auto inset-0 h-[calc(100%-1rem)] max-h-full absolute backdrop-blur-[1px] bg-opacity-40 bg-black-500 ${(width ?? 0) > breakpoints.lg ? switchHeaderClass() : ""}
 h-screen`}
    >
      <div className="relative w-full max-w-3xl max-h-full">
        <div className="relative animate-slide-down bg-white rounded-lg shadow dark:bg-gray-700 mt-[5rem] z-50">
          <Formik
            initialValues={{ activityName: data?.activityName || "" }}
            ...
            {({ isSubmitting, resetForm }) => (
              <Form>
                <div className="flex items-start justify-between p-4 rounded-t dark:border-gray-600">
                  <h3 className="font-semibold text-[15px] text-textPrimary-grey dark:text-white uppercase">{title ? "Edit Activity" : "Add Activity"}</h3>
                  <IoCloseSharp
                    size={30}
...
peer-disabled:translate-y-[-50%] peer-disabled:scale-75 peer-disabled:bg-[#fff] peer-disabled:px-1"
                        ...
                        <ErrorMessage component="div" name="activityName" className="text-red-500 text-sm" />
                      </div>
                    </div>
                  </div>
                </div>

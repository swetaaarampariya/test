...
import { AddEditInterestsProps } from "@/common/constants/interface";

const AddEditInterests: FC<AddEditInterestsProps> = ({ toggleModal, title, collapsed, data, setTableFilter, tableData }) => {
  ...
  const handleSubmit = async (values: { interestName: string }, { resetForm, setSubmitting }: any) => {
    try {
      const payload = { id: data?.id, interestName: values?.interestName };
     
      const response = title
        ? await AxiosPatch(`${API_URLS.INTERESTS.UPDATE}`, payload)
        : await AxiosPost(`${API_URLS.INTERESTS.ADD}`, payload);
  ...
  const existingInterestsName = tableData?.data?.map((item:any) => item.interestName) || [];

  const InterestsValidation = Yup.object().shape({
    interestName: Yup.string()
      .required('Interests name is required')
      .max(100, 'Interests name cannot exceed 100 characters')
      .test(
        'unique-interestName',
        'This Interests name already exists',
        (value) =>
        { if (data?.id && value === data?.interestName) {
          return true;
        }
        if(value && !existingInterestsName.includes(value)){
          return true;
        }
        }
      ),
    ...
    >
      <div className="relative w-full max-w-3xl max-h-full">
        <div className="relative animate-slide-down bg-white rounded-lg shadow dark:bg-gray-700 mt-[5rem] z-50">
          <Formik
            initialValues={{ interestName: data?.interestName || "" }}
            validationSchema={InterestsValidation}
            ...
            {({ isSubmitting, resetForm }) => (
              <Form>
                <div className="flex items-start justify-between p-4  rounded-t dark:border-gray-600">
                  <h3 className="font-semibold text-[15px] text-textPrimary-grey dark:text-white uppercase">{title ? "Edit Interests" : "Add Interests"}</h3>
                  <IoCloseSharp
                    size={30}
             ...
             peer-disabled:translate-y-[-50%] peer-disabled:scale-75 peer-disabled:bg-[#fff] peer-disabled:px-1"
                        ...
                        <ErrorMessage component="div" name="interestName" className="text-red-500 text-sm" />
                      </div>
                    </div>
                  </div>
                </div>
...
export default AddEditInterests;
